      ...
      <>
        {users.map((userItem) => (
          <div key={userItem.id} className="user-list-item flex v-center">
            ...
            <div className="user-name" title={userItem.name}>{userItem.name}</div>
        ...
        <div className="button-box flex v-center">
          ...
          <span>第 {page + 1} / {totalPage} 页</span>
        </div>
      </>
    )
}
...
export default observer(() => {
  ...
  useEffect(() => {
    ...
    Api.fetchTopicAllowedUsers({ offset: allowPage * pageLimit, limit: pageLimit }).then((result) => {
  ...
  useEffect(() => {
    setDenyLoading(true)
    Api.fetchTopicDeniedUsers({ offset: denyPage * pageLimit, limit: pageLimit }).then((result) => {
      if (denyPage !== 0 && result.count && !result.users.length) {
        setDenyPage(0)
        return
      }
  ...
  const handleConfirmChangeUserPermission = async () => {
    ...
    try {
      ...
      store.snackbar.open(`已${typeMap[type]} ${userName} 发布文章`, 2000)
      reloadData()
    } catch (e) {
      store.snackbar.open(`${typeMap[type]} ${userName} 发布文章失败！`, 2000, 'error')
    }
  }
